...
<!--ts-->
   * [Golang，译：Transparent Logs for Skeptical Clients](#golang译transparent-logs-for-skeptical-clients)
   * [标题：为持怀疑态度的客户端设计的透明日志](#标题为持怀疑态度的客户端设计的透明日志)
   * [目录](#目录)
   * [译前名词解释](#译前名词解释)
   * [正文](#正文)
   * [密码学中的哈希、验证和承诺](#密码学中的哈希验证和承诺)
   * [默克尔树（Merkle Trees）](#默克尔树merkle-trees)
   * [基于默克尔树结构的日志](#基于默克尔树结构的日志)
   * [存储日志](#存储日志)
   * [提供日志服务](#提供日志服务)
   * [验证日志](#验证日志)
   * [对日志进行瓦片化](#对日志进行瓦片化)
   ...
   * [总结](#总结)
   * [延伸阅读](#延伸阅读)
   * [附录 A：后序存储设计](#附录-a后序存储设计)
   * [附录 B：中序存储设计](#附录-b中序存储设计)
   * [附录 C：瓦片存储设计](#附录-c瓦片存储设计)
...
一个二叉树可被分割为固定的多个高为 H，宽为 <a href="https://www.codecogs.com/eqnedit.php?latex=\inline&space;\fn_cm&space;2^{H}" target="_blank"><img src="https://latex.codecogs.com/svg.latex?\inline&space;\fn_cm&space;2^{H}" title="2^{H}" /></a> 的瓦片。例如，下面是一棵拥有 27 个数据记录的永久哈希树，被分割为多个高度为 2 的瓦片：
