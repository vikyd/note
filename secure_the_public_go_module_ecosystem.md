...
<!--ts-->
   * [译：Proposal：Secure the Public Go Module Ecosystem](#译proposalsecure-the-public-go-module-ecosystem)
   * [提案：为 Go 语言的公共模块生态建立安全机制](#提案为-go-语言的公共模块生态建立安全机制)
   * [目录](#目录)
   * [译前名词解释](#译前名词解释)
   * [摘要](#摘要)
   * [背景](#背景)
      * [基于 go.sum 进行模块验证](#基于-gosum-进行模块验证)
      * [透明日志](#透明日志)
   * [提议](#提议)
      * [校验数据库（Checksum Database）](#校验数据库checksum-database)
      * [代理一个校验数据库](#代理一个校验数据库)
      * [go 命令客户端](#go-命令客户端)
   * [根本原因](#根本原因)
      * [安全](#安全)
      * [隐私](#隐私)
         * [私有模块的路径](#私有模块的路径)
         * [私有模块的 SHA256](#私有模块的-sha256)
         * [公共模块的使用数据](#公共模块的使用数据)
         * [通过代理保护隐私](#通过代理保护隐私)
         * [通过批量下载保护隐私](#通过批量下载保护隐私)
         * [在持续集成、持续开发（CI/CD）中的隐私](#在持续集成持续开发cicd中的隐私)
   * [兼容性](#兼容性)
   * [实现](#实现)
