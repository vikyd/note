...
<!--ts-->
   * [Golang Module 背后的安全与代理机制](#golang-module-背后的安全与代理机制)
   * [目录](#目录)
   * [安全是指什么？](#安全是指什么)
   * [名词约定](#名词约定)
   * [引用说明](#引用说明)
   * [实验环境](#实验环境)
   * [Go 1.13 中 go get 背后发生了什么？](#go-113-中-go-get-背后发生了什么)
      * [go get 1.13 vs 1.12](#go-get-113-vs-112)
      * [go 1.13 中 go get 抓包实验](#go-113-中-go-get-抓包实验)
         * [抓包准备](#抓包准备)
         * [开始实验](#开始实验)
         * [抓包结果](#抓包结果)
         * [结果简析](#结果简析)
         * [日常主要流程](#日常主要流程)
   * [Golang 关于 Module 安全的主要文章](#golang-关于-module-安全的主要文章)
   * [安全方案总览](#安全方案总览)
   * [中间层总览](#中间层总览)
      ...
      * [可能的疑惑](#可能的疑惑)
   * [服务分析](#服务分析)
   * [分析 proxy.golang.org](#分析-proxygolangorg)
      * [优点](#优点)
      * [缺点](#缺点)
      * [细节特点](#细节特点)
      * [接口：/modulePath/@v/list 版本列表](#接口modulepathvlist-版本列表)
      * [接口：/modulePath/@v/version.info 版本元数据信息](#接口modulepathvversioninfo-版本元数据信息)
      * [接口：/modulePath/@v/version.mod go.mod 文件](#接口modulepathvversionmod-gomod-文件)
      * [接口：/modulePath/@v/version.zip 模块源码](#接口modulepathvversionzip-模块源码)
      * [接口：/sumdb/databaseURL/supported 检查是否支持 sumdb 代理](#接口sumdbdatabaseurlsupported-检查是否支持-sumdb-代理)
      * [接口：/sumdb/databaseURL/sumdbSubUrl sumdb 的子路径](#接口sumdbdatabaseurlsumdbsuburl-sumdb-的子路径)
      * [第三方代理](#第三方代理)
         * [官方：proxy.golang.org](#官方proxygolangorg)
         * [七牛云：goproxy.cn](#七牛云goproxycn)
         * [个人？：goproxy.io](#个人goproxyio)
         * [腾讯云：mirrors.tencent.com/go](#腾讯云mirrorstencentcomgo)
         * [阿里云：mirrors.aliyun.com/goproxy](#阿里云mirrorsaliyuncomgoproxy)
         * [百度：goproxy.baidu.com](#百度goproxybaiducom)
   * [分析 sum.golang.org](#分析-sumgolangorg)
      * [概述](#概述)
      * [sumdb 原理简述](#sumdb-原理简述)
         * [细节问题](#细节问题)
      * [优点](#优点-1)
      * [缺点](#缺点-1)
      * [细节特点](#细节特点-1)
         * [sumdb 涉及技术](#sumdb-涉及技术)
      * [接口：/lookup/modulePath@version 模块版本哈希及子树签名（核心）](#接口lookupmodulepathversion-模块版本哈希及子树签名核心)
         * [模块哈希值的奇怪计算方法](#模块哈希值的奇怪计算方法)
         * [根哈希签名验证](#根哈希签名验证)
            * [数字签名回顾](#数字签名回顾)
            * [sumdb 签名](#sumdb-签名)
            * [公钥的哈希值](#公钥的哈希值)
            * [私钥的哈希值](#私钥的哈希值)
            * [签名的哈希值](#签名的哈希值)
            * [Ed25519 了解](#ed25519-了解)
         * [根哈希、树大小、模块编号](#根哈希树大小模块编号)
            * [叶子节点不是模块 zip 哈希值](#叶子节点不是模块-zip-哈希值)
            * [树大小为何经常变化](#树大小为何经常变化)
         * [瓦片 URL 推算](#瓦片-url-推算)
            * [瓦片 URL 组成](#瓦片-url-组成)
            * [瓦片 URL 特殊规则](#瓦片-url-特殊规则)
            * [推算模块哈希所在瓦片](#推算模块哈希所在瓦片)
            * [推算叶子层最右侧瓦片](#推算叶子层最右侧瓦片)
            * [推算其他层的瓦片](#推算其他层的瓦片)
         * [验证模块哈希可信](#验证模块哈希可信)
      * [接口：/tile/tileHeight/tileLevel/offsetInLevel 哈希值完整瓦片](#接口tiletileheighttileleveloffsetinlevel-哈希值完整瓦片)
      * [接口：/tile/tileHeight/tileLevel/offsetInLevel.p/tileWidth 哈希值不完整瓦片](#接口tiletileheighttileleveloffsetinlevelptilewidth-哈希值不完整瓦片)
      * [接口：/tile/tileHeight/data/offsetInLevel0 叶子完整瓦片对应模块哈希](#接口tiletileheightdataoffsetinlevel0-叶子完整瓦片对应模块哈希)
      * [接口：/tile/tileHeight/data/offsetInLevel0.p/tileWidth 叶子不完整瓦片对应模块哈希](#接口tiletileheightdataoffsetinlevel0ptilewidth-叶子不完整瓦片对应模块哈希)
      * [接口：/latest 最新根哈希](#接口latest-最新根哈希)
      * [第三方 sumdb](#第三方-sumdb)
      * [sum.golang.org 网站是否开源？](#sumgolangorg-网站是否开源)
   * [分析 index.golang.org](#分析-indexgolangorg)
      * [接口：/index](#接口index)
   * [GOPROXY、GOSUMDB、GOPRIVATE 等的用法](#goproxygosumdbgoprivate-等的用法)
      ...
      * [GONOPROXY](#gonoproxy)
      * [GONOSUMDB](#gonosumdb)
      * [GOPRIVATE、GONOPROXY、GONOSUMDB 的公共特点](#goprivategonoproxygonosumdb-的公共特点)
      * [修改这些值的方式](#修改这些值的方式)
      * [几种组合方式的含义](#几种组合方式的含义)
         * [全部默认](#全部默认)
         * [仅设置 GOPRIVATE](#仅设置-goprivate)
      ...
      * [推荐配置](#推荐配置)
         * [公司有模块代理且代理了 sumdb](#公司有模块代理且代理了-sumdb)
         * [公司有模块代理但没代理 sumdb](#公司有模块代理但没代理-sumdb)
         * [公司无模块代理也无代理 sumdb](#公司无模块代理也无代理-sumdb)
         * [在家里](#在家里)
   * [未解的疑问](#未解的疑问)
   * [总结](#总结)
   * [参考](#参考)
